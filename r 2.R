...
data=c(2,4,6,8);data
sample.data=rbind(
c(2,2),c(2,4),c(2,6),c(2,8),
c(4,2),c(4,4),c(4,6),c(4,8),
c(6,2),c(6,4),c(6,6),c(6,8),
...
n1=length(X);n1
n2=length(Y);n2
stadeviationX=sqrt(Xvar);stadeviationX
stadeviationY=sqrt(Yvar);stadeviationY
...
exp=c(0.46, 0.38, 0.61, 0.82, 0.59, 0.53, 0.72, 0.44, 0.58, 0.60, 0.73, 0.55, 0.23, 0.62, 0.38, 0.27, 0.36,0.47, 0.49, 0.71);exp
mean=mean(exp);mean

(i)
n=length(exp);n
muhat=sum(exp)/n;muhat
sigmahat=sqrt((sum(exp^2)-n*muhat^2)/n);sigmahat
...
t_value= qt(1 - alpha,19 , lower.tail = TRUE);t_value
LCL=muhat-abs(t_value)*(sigmahat/sqrt(n));LCL
UCL=muhat+abs(ztab)*sqrt(sigmahat/n);UCL

(iii)
variance=1/(thetahat)^2;variance
...
lCI=xbar-ztab*sd/sqrt(n);lCI
uCl=xbar+ztab*sd/sqrt(n);uCl
...
write.csv(data, file = 'C:/Users/Hp/Downloads/stat/data1.csv', row.names = FALSE);
alpha=0.05;alpha
n1=length(temp1);n1
n2=length(temp2);n2
...
data=read.csv(file.choose('C:/Users/Hp/Downloads/stat/ab.csv'));data
math = data[,3];math
stat = data[,2];stat
...
data=cbind(X,Y);data
m=data.frame(X,Y);m
write.csv(data, file = 'C:/Users/Hp/Downloads/stat/data2.csv', row.names = FALSE);
alpha=0.05;alpha
